...
class GrokAiService {
  static _apiKey = process.env.GROKAI_API_KEY!

  private static get apiKey() {
    if (!this._apiKey) {
      this._apiKey = process.env.GROKAI_API_KEY!;
    }
    return this._apiKey;
  }

  static async analyzeTrend(image: Buffer): Promise<TAiCandleTrendDirection> {
    try {
...
Your job is NOT to speculate about the future, you only say what the image (the current presence) shows you. "Up" or "Down" or "Kangaroo" only.`,
  ...
  static async analyzeShouldHoldOrResolve(image: Buffer): Promise<"Hold" | "Resolve"> {
    try {
      const base64Image = image.toString('base64');
...
`,
  ...
  static async analyzeBreakoutTrendV2(image: Buffer): Promise<"Up" | "Down" | "Kangaroo"> {
    try {
...
If unsure, choose "Kangaroo".`,
  ...
  static async analyzeBreakoutTrendWithAfter(image: Buffer): Promise<"Up" | "Down" | "Already-Up" | "Already-Down"> {
    try {
